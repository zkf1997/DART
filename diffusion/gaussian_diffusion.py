...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if not self.ignore_history and 'history_mask' in model_kwargs['y'].keys() and 'history_motion' in model_kwargs['y'].keys():
    ...
    def training_losses_old(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        """
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None,
                        body_model_male=None, body_model_female=None,
                        return_output=False, clean_validation=False,
                        # ignore_history=False
                        ):
        """
        Compute training losses for a single timestep.

        :param model: the model to evaluate loss on. Note this is wrapped model after respacing.
        :param x_start: the [N x C x ...] tensor of inputs.
        :param t: a batch of timestep indices.
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
        ...
        if not self.ignore_history:
            history_mask = model_kwargs['y']['history_mask']
            history_length = model_kwargs['y']['history_length']
            future_length = model_kwargs['y']['future_length']
            history_motion = model_kwargs['y']['history_motion']

        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start)
        # noise.requires_grad_(True)  # for smooth diffusion regularization

        x_t = self.q_sample(x_start, t, noise=noise)
        if not self.ignore_history:
            x_t = (x_t * ~history_mask) + (history_motion * history_mask)  # enforce the history motion frames comes from input rollout seed
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            if not clean_validation:
                model_output = model(x_t, self._scale_timesteps(t), **model_kwargs)
                if not self.ignore_history:
                    model_output = (model_output * ~history_mask) + (history_motion * history_mask)  # enforce the history motion frames comes from input rollout seed
            else:
                # Note this is wrapped model after respacing.
                # print(type(model))
                sample_fn = self.p_sample_loop
                model_output = sample_fn(
                    model,
                    x_start.shape,
                    clip_denoised=False,
                    model_kwargs=model_kwargs,
                ...
                # sample loop already done history infilling
            ...
            # tensor denoise loss
            # terms["future_tensor_mse"] = self.masked_l2(target, model_output, ~history_mask) # mean_flat(rot_mse)
            if self.ignore_history:
                terms["future_tensor_mse"] = mean_flat(self.mse_loss(target, model_output))  # mean_flat(rot_mse)
            else:
                terms["future_tensor_mse"] = mean_flat(self.mse_loss(target[:, :, :, history_length:], model_output[:, :, :, history_length:]))  # mean_flat(rot_mse)

            """smplx consistency losses"""
            gt_motion_tensor = x_start if self.ignore_history else x_start[:, :, :, history_length:]  # skip the history frames, # [bs, D, 1, nframes_future]
            gt_motion_tensor = gt_motion_tensor.squeeze(dim=2).permute(0, 2, 1)  # [bs, nframes_future, D]
            pred_motion_tensor = model_output if self.ignore_history else model_output[:, :, :, history_length:]
            pred_motion_tensor = pred_motion_tensor.squeeze(dim=2).permute(0, 2, 1)  # [bs, nframes_future, D]
            ...
            def get_smpl_body(motion_tensor, genders, betas):
                ...
                for gender_name in ['female', 'male']:
                    ...
                    if self.ignore_history:
                        gender_betas = betas[gender_idx, :, :].reshape(
                    ...
                    gender_motion_tensor = dataset.denormalize(gender_motion_tensor).reshape(sub_batch_size * num_frames, -1)
                    # transl = gender_motion_tensor[:, :3]
                    # poses_6d = gender_motion_tensor[:, 3:3+22*6]
                    # joints.append(gender_motion_tensor[:, 3+22*6+3+6:3+22*6+3+6+22*3].reshape(sub_batch_size, num_frames, 22, 3))  # [bs, nframes, 22, 3]
                    # smplxout = body_model(return_verts=True,
                    #                       betas=gender_betas,
                    #                       transl=transl,
                    #                       global_orient=transforms.rotation_6d_to_matrix(poses_6d[:, :6]),
                    #                       )
                    ...
                    smplxout = body_model(return_verts=True, **smplx_param)
                    smpl_joints.append(smplxout.joints[:, :22, :].reshape(sub_batch_size, num_frames, 22, 3))  # [bs, nframes, 22, 3]
                    smpl_vertices.append(smplxout.vertices.reshape(sub_batch_size, num_frames, -1, 3))  # [bs, nframes, V, 3]
            ...
            with torch.no_grad():
                gt_result_dict = get_smpl_body(gt_motion_tensor, genders, betas)  # note that each batch is reordered according to gender. we assume the input batch is already sorted by gender, so the actual order does not change after this operation
            pred_result_dict = get_smpl_body(pred_motion_tensor, genders, betas)
            terms['smpl_joints_mse'] = mean_flat(self.mse_loss(gt_result_dict['smpl_joints'], pred_result_dict['smpl_joints']))
            """ use vertex loss leads to cuda out of memory! """
            # terms['smpl_vertices_mse'] = mean_flat(self.mse_loss(gt_result_dict['smpl_vertices'], pred_result_dict['smpl_vertices']))
            terms['joints_consistency_mse'] = mean_flat(self.mse_loss(pred_result_dict['smpl_joints'], pred_result_dict['joints']))
            # terms['smpl_joints_mse'] = torch.tensor(0.)
            terms['smpl_vertices_mse'] = torch.zeros_like(terms['future_tensor_mse'])
            # terms['joints_consistency_mse'] = torch.tensor(0.)

            """temporal delta loss"""
            if self.ignore_history:
                pred_motion_tensor = model_output.squeeze(2).permute(0, 2, 1)  # [B, T, D]
            else:
                pred_motion_tensor = model_output[:, :, :, history_length-1:].squeeze(2).permute(0, 2, 1)  # [B, T, D]
            ...
            joints_delta_mse = self.mse_loss(pred_joints_delta, calc_joints_delta)  # [B, F, 22*3]
            transl_delta_mse = self.mse_loss(pred_transl_delta, calc_transl_delta)  # [B, F, 3]
            orient_delta_mse = self.mse_loss(pred_orient_delta, calc_orient_delta_6d)  # [B, F, 6]
            terms["first_joints_delta_mse"] = mean_flat(joints_delta_mse[:, 0, :])
            terms["first_transl_delta_mse"] = mean_flat(transl_delta_mse[:, 0, :])
            terms["first_orient_delta_mse"] = mean_flat(orient_delta_mse[:, 0, :])
            terms["joints_delta_mse"] = mean_flat(joints_delta_mse)
            terms["transl_delta_mse"] = mean_flat(transl_delta_mse)
            terms["orient_delta_mse"] = mean_flat(orient_delta_mse)
            terms["rel_joints_delta_mse"] = mean_flat(joints_delta_mse / (pred_joints_delta ** 2).clamp(min=1e-6))
            terms["rel_transl_delta_mse"] = mean_flat(transl_delta_mse / (pred_transl_delta ** 2).clamp(min=1e-6))
            # terms["rel_orient_delta_mse"] = mean_flat(orient_delta_mse / (pred_orient_delta ** 2).clamp(min=1e-6))
            terms["rel_orient_delta_mse"] = torch.zeros_like(terms['smpl_joints_mse'])

            B, T, _ = pred_joints_delta.shape
            terms["delta_norm"] = mean_flat(
                torch.norm(pred_joints_delta.reshape(B, T, 22, 3), dim=-1).mean(dim=-1) +
                torch.norm(calc_joints_delta.reshape(B, T, 22, 3), dim=-1).mean(dim=-1) + \
                torch.norm(pred_transl_delta, dim=-1) + torch.norm(calc_transl_delta, dim=-1)
                                   )

            # dct frequency loss
            if self.ignore_history:
                gt_motion_tensor = x_start
                pred_motion_tensor = model_output
            else:
                gt_motion_tensor = x_start[:, :, :, history_length:]  # skip the history frames, # [bs, D, 1, nframes_future]
                pred_motion_tensor = model_output[:, :, :, history_length:]
            gt_dct = dct.dct(gt_motion_tensor)   # DCT-II done through the last dimension, https://github.com/zh217/torch-dct
            pred_dct = dct.dct(pred_motion_tensor)
            # print('gt_dct:', gt_dct.shape, 'pred_dct:', pred_dct.shape)
            terms['dct_mse'] = mean_flat(self.mse_loss(gt_dct, pred_dct))  # mean_flat(rot_mse)

            # jerk normalization loss
            if model_output.shape[-1] > 3:
                pred_motion_tensor = model_output.squeeze(2).permute(0, 2, 1)  # [B, T, D]
                pred_motion_tensor = dataset.denormalize(pred_motion_tensor)
                pred_feature_dict = dataset.tensor_to_dict(pred_motion_tensor)
                pred_joints = pred_feature_dict['joints']  # [B, T, 22*3]
                pred_joints = pred_joints.reshape(*(pred_joints.shape[:2]), 22, 3)  # [B, T, 22, 3]
                # Calculate the third derivative of the poses with respect to time to get the jerk
                vel = pred_joints[:, 1:] - pred_joints[:, :-1]  # --> B x T-1 x 22 x 3
                acc = vel[:, 1:] - vel[:, :-1]  # --> B x T-2 x 22 x 3
                jerk = acc[:, 1:] - acc[:, :-1]  # --> B x T-3 x 22 x 3
                # print('jerk:', jerk.shape)
                terms['jerk'] = mean_flat(jerk ** 2)
            else:
                terms['jerk'] = torch.zeros_like(terms['smpl_joints_mse'])


            # smooth diffusion regularization: https://github.com/SHI-Labs/Smooth-Diffusion/blob/5522761bb68fcb6ac1cfaee5a5b855d4a56ea33f/train_smooth_diffusion.py#L738
            # def step_regularize(fake_img, latents, mean_reg_variation, sqrt_one_minus_alpha_prod, decay=0.01):
            #     # print(fake_img.requires_grad)
            #     sample_shape = list(fake_img.shape)
            #     sample_shape[-1] = future_length
            #     num_dim = len(sample_shape)
            #
            #     noise = torch.randn_like(fake_img) / math.sqrt(
            #         torch.tensor(sample_shape[1:]).prod().float()  # skip batch dim
            #     )
            #     noise = noise * sqrt_one_minus_alpha_prod
            #     output_sum = (fake_img * noise * ~history_mask).sum()  # only consider the future frames
            #     grad, = autograd.grad(
            #         outputs=output_sum, inputs=latents, create_graph=True,
            #     )
            #     reg_variations = torch.sqrt(grad.pow(2).sum(dim=list(range(1, num_dim))))
            #
            #     variation_mean = mean_reg_variation + decay * (reg_variations.mean() - mean_reg_variation)
            #
            #     variation_penalty = (reg_variations - variation_mean).pow(2).mean()
            #
            #     return variation_penalty, variation_mean.detach(), reg_variations
            #
            # reg_loss, self.mean_reg_variation, reg_variations = step_regularize(
            #     model_output, noise, self.mean_reg_variation,
            #     _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, model_output.shape)
            # )
            # terms["reg_loss"] = reg_loss
            # terms["reg_variations"] = reg_variations
            # terms["mean_reg_variation"] = self.mean_reg_variation

            # print(terms['future_tensor_mse'].shape, terms['smpl_joints_mse'].shape, terms['smpl_vertices_mse'].shape, terms['joints_consistency_mse'].shape)
            diffusion_step_mask = (t < self.num_timesteps * self.mask_thresh_step).float()  # use auxiliary losses only for the cleaner steps, assuming the most noisy steps do not have meaning kinematic and dynamic
            # print(f"steps: {t} diffusion_step_mask: {diffusion_step_mask}")
            # print(terms["future_tensor_mse"].shape)
            for key in terms.keys():
                if key != 'future_tensor_mse':
                    terms[key] = terms[key] * diffusion_step_mask
            auxiliary_losses = (self.lambda_smpl_joints * terms['smpl_joints_mse']) + \
                               (self.lambda_smpl_vertices * terms['smpl_vertices_mse']) + \
                               (self.lambda_joints_consistency * terms['joints_consistency_mse']) + \
                               (self.lambda_joints_delta * terms['joints_delta_mse']) + \
                               (self.lambda_transl_delta * terms['transl_delta_mse']) + \
                               (self.lambda_orient_delta * terms['orient_delta_mse']) + \
                               (self.lambda_first_joints_delta * terms['first_joints_delta_mse']) + \
                               (self.lambda_first_transl_delta * terms['first_transl_delta_mse']) + \
                               (self.lambda_first_orient_delta * terms['first_orient_delta_mse']) + \
                               (self.lambda_rel_joints_delta * terms['rel_joints_delta_mse']) + \
                               (self.lambda_rel_transl_delta * terms['rel_transl_delta_mse']) + \
                               (self.lambda_rel_orient_delta * terms['rel_orient_delta_mse']) + \
                               (self.lambda_delta_norm * terms['delta_norm']) + \
                               (self.lambda_dct * terms['dct_mse']) + \
                               (self.lambda_jerk * terms['jerk'])  # [B,]
            terms["loss"] = terms["future_tensor_mse"] + auxiliary_losses

            # print('lamda_delta_norm:', self.lambda_delta_norm, terms['delta_norm'])
            # print('weight_smpl_joint', self.lambda_smpl_joints)
            # print('weight_joint_consistency', self.lambda_joints_consistency)
