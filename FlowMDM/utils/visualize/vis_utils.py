...
class npy2obj:
    ...
    def save_npz(self, save_path):
        thetas = torch.tensor(self.motions['motion'][0, :-1, :, :self.real_num_frames])
        thetas = thetas.permute(2, 0, 1)
        thetas = thetas[:, :22, :6].reshape(-1, 6)
        thetas = matrix_to_axis_angle(rotation_6d_to_matrix(thetas)).reshape(self.real_num_frames, 22 * 3)  # [T, 22*3]
        thetas = torch.cat([thetas, torch.zeros(self.real_num_frames, 99).to(dtype=thetas.dtype, device=thetas.device)], dim=1)
        trans = torch.tensor(self.motions['motion'][0, -1, :3, :self.real_num_frames])
        trans = trans.permute(1, 0)  # [T, 3]
        data_dict = {
